...
let export path graph =
  (* Open a write-file. *)
  let writefile = open_out path in

  let create_line id1 id2 v_str =
  ...
  (* Write in this file. *)
  fprintf writefile "digraph finite_state_machine {\n" ;

  fprintf writefile "\trankdir=LR; \n\tsize=\"8,5\"\n";

  fprintf writefile "\tnode [shape = circle]; \n";

  e_iter graph (create_line);
